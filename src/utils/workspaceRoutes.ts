...
/**
 * 生成 Platform App 内部页面 URL。
 * appKey 来自后端授权，pageKey 来自 appRegistry.navigation.pathSegment。
 */
...
/**
 * 生成 Organization App 内部页面 URL；organizationId 保证不同组织不能共享标签快照。
 */
...
/**
 * 登录落点的单一公开门面。
 * 调用方先完成认证判断；登录提交、应用根路由和旧 /home 兼容入口共用该路径。
 */
export const resolveLandingPath = () => getPlatformHomePath();
...
/**
 * 从 App 通配路由读取 App 内部页面 Key；App 根路径返回 undefined。
